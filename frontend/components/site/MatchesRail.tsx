  ...
  return (
    <div className="flex flex-col gap-2.5 rounded-card border border-line bg-paper p-4">
      <div className="flex items-center justify-between gap-2">
        <span className="truncate text-[11.5px] font-semibold text-ink-3">{match.round_label || match.league}</span>
        {live ? (
      ...
      <div className="flex items-center gap-3">
        <span className="min-w-0 flex-1 truncate text-[14px] font-bold text-ink">{match.home_team}</span>
        <span
        ...
        <span className="min-w-0 flex-1 truncate text-end text-[14px] font-bold text-ink">{match.away_team}</span>
      </div>

      {!finished && match.kickoff_at ? (
        <div className="tnum text-center text-[11.5px] text-ink-3">{kickoffLabel(match.kickoff_at)}</div>
      ) : match.venue ? (
        <div className="truncate text-center text-[11.5px] text-ink-3">{match.venue}</div>
      ) : null}
    </div>
  );
}
