...
/**
 * English counterpart of /reel/[slug] — same reel by slug, English chrome.
 * See the Arabic page for the reasoning behind every choice here.
 */
...
export default async function ReelEnPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const others = othersRes.results
    ...
    .slice(0, 8)
    .map((r) => ({ id: r.id, title: r.title, thumbnail: mediaUrl(r.thumbnail), href: `/en/reel/${r.slug}`, youtubeId: r.youtube_id }));

  const t = REEL_COPY.en;
      ...
      <section className="bg-board py-8">
        <div className="mx-auto max-w-container px-6">
          ...
          <ReelPlayer
            ...
            shareUrl={`${SITE_URL}/en/reel/${encodeURIComponent(reel.slug)}`}
            watchUrl={reel.url}
          />
