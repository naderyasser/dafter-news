...
export default async function HomeEnPage() {
  ...
  const opinionItems = opinion.results.map((a) => ({ name: a.author_name || "", quote: a.title, href: `/opinion/${a.slug}`, initial: a.author_initial || "?" }));
  ...
  return (
    <SiteShell lang="en" active="home">
      ...
      <OpinionCarousel lang="en" items={opinionItems} seeAllHref="/opinion" />
          ...
          <div className="rounded-card border border-line bg-paper p-5">
            ...
            <div className="mt-3.5 flex flex-wrap gap-2">
              {tags.results.slice(0, 5).map((t) => (
                <Link key={t.id} href="#" className="rounded-pill bg-brand-tint px-3.5 py-1.5 text-[13px] font-semibold text-brand no-underline hover:bg-brand hover:text-paper">
                  {t.name}
                </Link>
              ))}
            </div>
